...
import { checkOwner, isHouseholdResolved, reverseDate } from '../../helpers/rendering';
...
const HouseholdTransactions = ({ householdInfo }) => {
  ...
  const checkMyBill = () => {
    const billArray = []
    const notCreated = recent.filter(ex => ex.creator !== id)
  ...
  const checkWhatIAmOwed = () => {
    const billArray = []
    const Created = recent.filter(ex => ex.creator === id)
    ...
    <section className="h-full flex flex-col justify-evenly">
      {!showUnresolved ? (
        <>
          ...
          {
            showOwed ? <p>You're owed £{checkWhatIAmOwed()}</p> : <p>show me what I'm owed</p>
          }
          ...
          >
            {
              showBill ? <p>You owe £{checkMyBill()}</p> : <p>show me what I owe</p>
            }
          </div>
          </div>
        </>
